#ifndef WCN_ARM_NEON_ATOMIC_H
#define WCN_ARM_NEON_ATOMIC_H
...
/* ARM NEON atomic operations with proper memory barriers */

/* ========== Memory Ordering Helpers ========== */

WCN_INLINE void wcn_arm_memory_fence(wcn_memory_order_t order) {
    switch (order) {
...
/* ARM NEON atomic loads with memory barriers */
...
/* ARM NEON atomic stores with memory barriers */
...
/* Aligned atomic operations (same as unaligned for NEON) */
...
/* ARM NEON atomic compare-and-exchange using scalar CAS loop */
WCN_INLINE int wcn_v128i_atomic_compare_exchange_strong(wcn_v128i_t* ptr, wcn_v128i_t* expected,
    ...
    if (success_order == WCN_MEMORY_ORDER_SEQ_CST || failure_order == WCN_MEMORY_ORDER_SEQ_CST ||
        ...
        wcn_arm_memory_fence(WCN_MEMORY_ORDER_ACQ_REL);
    }
    ...
    if (success) {
        if (success_order == WCN_MEMORY_ORDER_SEQ_CST) {
            wcn_arm_memory_fence(WCN_MEMORY_ORDER_SEQ_CST);
        } else if (success_order == WCN_MEMORY_ORDER_RELEASE) {
            wcn_arm_memory_fence(WCN_MEMORY_ORDER_RELEASE);
        }
    } else {
...
/* Weak CAS is the same as strong for NEON */
