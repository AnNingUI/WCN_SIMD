#ifndef WCN_X86_SSE2_ATOMIC_H
#define WCN_X86_SSE2_ATOMIC_H
...
/* x86 SSE2 has limited atomic SIMD support - implement using scalar fallback with locks */

/* ========== Memory Ordering Helpers ========== */

WCN_INLINE void wcn_x86_memory_fence(wcn_memory_order_t order) {
    switch (order) {
...
/* SSE2 doesn't have true atomic SIMD loads/stores - use scalar fallback */
WCN_INLINE wcn_v128i_t wcn_v128i_atomic_load(const wcn_v128i_t* ptr, wcn_memory_order_t order) {
    wcn_v128i_t result;
    wcn_x86_memory_fence(order);
...
/* Aligned atomic operations (same as unaligned for SSE2) */
...
/* SSE2 doesn't have native atomic SIMD CAS - implement using scalar CAS loop */
WCN_INLINE int wcn_v128i_atomic_compare_exchange_strong(wcn_v128i_t* ptr, wcn_v128i_t* expected,
    ...
    if (success) {
        wcn_x86_memory_fence(success_order);
    } else {
        wcn_x86_memory_fence(failure_order);
    }
...
/* Weak CAS is the same as strong for SSE2 */
